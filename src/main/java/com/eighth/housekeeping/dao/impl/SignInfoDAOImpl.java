...
public class SignInfoDAOImpl extends BaseDAO implements SignInfoDAO {
    ...
    public SignInfo findTodaySignInfoByAuntId(String auntId) {
        ...
        List<SignInfo> signInfoList = getJdbcTemplate().query(sql.toString(),new String[]{auntId,todayStr+"%"},new SignInfoRowMapper());
    ...
    public SignInfo findSignDetail(String auntId) {
        ...
        StringBuilder sql = new StringBuilder("select count(*) from t_sign_info where aunt_id = ? and opt_time like ?");
        int counts = getJdbcTemplate().queryForObject(sql.toString(),new String[]{auntId,currentMonth},Integer.class);

        StringBuilder selectSql = new StringBuilder("SELECT max(opt_time) from t_sign_info where aunt_id = ? ");
        String lastSignDate = getJdbcTemplate().queryForObject(selectSql.toString(),new String[]{auntId},String.class);
